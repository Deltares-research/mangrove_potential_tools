...
import glob
import time
import rioxarray # This package must be here for correct computation of rio methods
import pandas as pd
...
esa_worldcover_urban_class = config["esa_worldcover_urban_class"]
time_logfile = config["time_logfile"]

# Define tiles and output directory
tiles_dir = os.path.join(data_dir, '1_Tiles', analysis_id)
output_dir = os.path.join(data_dir, '10_Landcover', analysis_id)
...
for tile_path in glob.glob(os.path.join(tiles_dir, '*_0.geojson')):
    ...
    try:
        ...
        binary_mask = (map_data == esa_worldcover_urban_class).astype('uint8')
