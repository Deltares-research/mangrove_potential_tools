...
tides_dir = os.path.join(data_dir, '3_Tides', analysis_id)
elevation_dir = os.path.join(data_dir, '4_Elevation', analysis_id)
output_dir = os.path.join(data_dir, '5_Accommodation_space', analysis_id)
...
for tide_path in glob.glob(os.path.join(tides_dir, '*.tif')):
    ...
    if not os.path.exists(tide_path):
        print(f"✘ Missing tide raster: {tide_path}. Skipping {tide_id}.")
        log.append({"tile_id": tide_id, "missing_file": tide_path})
        continue
...
log_csv_path = os.path.join(output_dir, f"MSL_HAT_BEY.csv")
